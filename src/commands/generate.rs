...
#[derive(Debug, Args)]
#[command(group(ArgGroup::new("_type").required(true)))]
pub struct Generate {
...
impl Generate {
    pub fn run(&self, path_string: &String) -> Result<()> {
        ...
        if self.passphrase {
            _password = self.generate_passphrase(&mut rng)?;
        }

        if self.password {
            _password = self.generate_password(&mut rng)?;
        }
        ...
        if let Some(name) = &self.name {
            ...
            entry.insert("password".to_string(), _password);
            ...
            store.encrypt(format!("{}", entry_file.display()), name, &entry)?;
            store.save_index()?;
    ...
    pub fn generate_passphrase(&self, rng: &mut ThreadRng) -> Result<String> {
        ...
        while length > 0 {
            match EFF_LARGE_WORDLIST.choose(rng) {
                Some(word) => {
                    passphrase.push(' ');
    ...
    pub fn generate_password(&self, rng: &mut ThreadRng) -> Result<String> {
        ...
        if !self.omit.contains(&"special".to_string()) {
            characters.push_str(SPECIAL_CHARACTERS);
        }

        if !self.omit.contains(&"number".to_string()) {
            characters.push_str(NUMERIC_CHARACTERS);
        }

        if !self.omit.contains(&"letter".to_string()) {
            characters.push_str(LETTERS);
        }
        ...
        while length > 0 {
            match characters.to_owned().choose(rng) {
                Some(c) => {
                    password.push(c);
                }
                None => {

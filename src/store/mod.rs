...
pub struct RpassStore {
    /// Absolute path to a secrets folder
    pub store: PathBuf,

    /// Run without writing any changes to the store.
    pub dry_run: bool,

    /// Asymmetric public key IDs or owner emails  for encrypting/decrypting secrets
    pub keys: Option<Vec<String>>,

    /// Uuid (actual) to secret group/name mapping
    pub map: AHashMap<String, secret::RpassSecret>,
}
...
impl RpassStore {
    ...
    pub fn from_cli(cli_args: RpassCli) -> Result<(), RpassError> {
        ...
        match &cli_args.command {
            ...
            Some(cmd) => match cmd {
                RpassCliCommand::Init { keys, git } => {}
                RpassCliCommand::Insert {
                    custom_fields,
                    echo,
                    name,
                } => {}
                RpassCliCommand::List { name, store } => {}
                RpassCliCommand::Show {
                    name,
                    fields,
                    clipboard,
                    wait,
                } => {}
                RpassCliCommand::Edit {
                    fields,
                    values,
                    new,
                    echo,
                ...
                RpassCliCommand::Remove { name } => {}
                RpassCliCommand::Generate {
                    length,
                    password,
                    omit,
                    passphrase,
                ...
                RpassCliCommand::Git { args } => {}
                RpassCliCommand::Copy { from, to } => {}
                RpassCliCommand::Move { from, to } => {}
            },
            None => {
                // Invoke the default subcommand depending on the passed arguments
